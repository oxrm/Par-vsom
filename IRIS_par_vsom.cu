...
///////////////////////////////////////////////////////////////////////////////
//Init Clocks to measure total time_elapsed
...
//Define device iterators to create tuple (4 colums matrix)
...
//Vector D_coor_d  (holds all the distances)
...
//vector to contain the nei (1,0)
thrust::device_vector<float> hood_vec_d_dim(N*4,0);

//Mask vector in GPU
thrust::device_vector<float> mask_vec_d(N,0);

//Neghborhood cache (Huge vector containing the nei for each instance)
thrust::device_vector<float> nei_cache(new_n * N,0);

//Vector for Eta values
thrust::device_vector<float> Eta_d(N*4,0);
// Fill Eta vector with eta contant value
...
Float4Iterator_d first_M1= thrust::make_zip_iterator(thrust::make_tuple(M_vector_d.begin(),M_vector_d.begin() + (N * 1),M_vector_d.begin() + (N * 2),M_vector_d.begin() + (N*3)));
...
//Randow variable selection for Xk (use size of dataset + 1)
int v1 = rand() % 149 + 1;
...
Float4Iterator_d vector_first_D = thrust::make_zip_iterator(thrust::make_tuple(P_vector_d.begin(),P_vector_d.begin() + (N * 1),P_vector_d.begin() + (N * 2),P_vector_d.begin() + (N*3)));
Float4Iterator_d vector_last_D  = thrust::make_zip_iterator(thrust::make_tuple(P_vector_d.begin() + (N * 1), P_vector_d.begin() + (N * 2) ,P_vector_d.begin() + (N *3),P_vector_d.begin() + (N*4) ));
...
//Select ramdow training instance index
v1 = rand() % 149 + 1;


//Fill vector X with Xk values X <-- 1^n * Xk
thrust::fill(X_vector_d.begin(), X_vector_d.begin() + (N * 1) ,file_vector_d[v1]);
thrust::fill(X_vector_d.begin() + (N * 1) , X_vector_d.begin() + (N * 2) ,file_vector_d[150 * 1 + v1]);
thrust::fill(X_vector_d.begin() + (N * 2) , X_vector_d.begin() + (N * 3) ,file_vector_d[150 * 2 + v1]);
thrust::fill(X_vector_d.begin() + (N * 3) , X_vector_d.begin() + (N * 4) ,file_vector_d[150 * 3 + v1]);
...
thrust::transform(D_vector_d.begin() + ( N * 3), D_vector_d.end(), hood_vec_d.begin(), D_vector_d.begin()+ ( N * 3), thrust::multiplies<float>());
...
if (epocs == train-1)
  {
   ...
   char filename[] = "iris_GPU_neurons";
   char underScore[] = "_";
   char txt[] = ".txt";
   char x_dim[33];
   char y_dim[33];
...
//Close neuron weight file
neuronsFile.close();

//Print out total training time in ms
